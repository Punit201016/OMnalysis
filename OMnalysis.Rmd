...
h6(strong("2. Currently support four species (Human, Cow, Pig, Chicken, Mouse, Rat, Dog, Drosophila melanogaster and C.elegans)."))
...
div(style = "width: 150px; margin-top: 20px; margin-buttom: 0px;", actionButton(inputId = "uploaded_ex", "Click to upload transcritomics example data"))

div(.progress = {"height: 0px; margin-top: 10px; margin-buttom: 0px;"}, fileInput(inputId = "inFile", label = h5(strong("Upload transcriptomics data")),
...
tags$hr(style="height:10px; border-width:1px; border-color: Black;")

div(style = "width: 150px;", actionButton(inputId = "uploaded_ex1", "Click to upload Proteomics example data"))

div(.progress = {"height: 0px; margin-buttom: 0px;"}, fileInput(inputId = "inFile1", label = h5(strong("Upload proteomics data")),
multiple = TRUE, accept = c(
                    "text/csv",
                    "text/comma-separated-values",
                    "text/plain",
...
actionLink("reset_1", strong("Click me to delete the uploaded data."))

tags$hr(style="height: 0px; border-width:3px; border-color: Black;")

div(h6(tags$b("After uploading the data, select the species, choose the ID conversion and click submit."), style = "display: inline;color: #000000;"))

div(style = "margin-bottom: 0px;", selectInput("select1", label = h5 (strong("Select a species")), choices = c("Human" = 1, "Chicken"= 2, "Pig"= 3, "Cow"= 4, "Mouse"= 5, "Rat" = 6, "Dog" = 7, "Drosophilla" = 8, "C.elegans" = 9), selected = NULL))

div(style = "display: inline-block; width: 150px; margin-top: 0px; margin-buttom: 0px", selectInput("select2", label = h5(strong("ID conversion")), choices = c("Ensembl gene ID", "UniProtKB/Swiss-Prot ID")))

div(style = "display: inline-block; width: 150px; margin-top: 0px; margin-buttom: 0px", selectInput("select3", label = "", choices = c("Ensembl gene ID" = 1, "Gene name"= 2, "HGNC symbol"= 3, "Gene description" = 4, "UniProtKB/Swiss-Prot ID" = 5), selected = NULL)%>% 
    shinyInput_label_embed(
    shiny::icon("info") %>%
    bs_embed_tooltip(title = "Converted IDs will be visible on PCA and volcano plot"))
)

div(actionButton("ID_submit", "Submit"), h6(tags$b("(After submission, click on PCA or Plots or Statistical filtering)."), style = "display: inline;color: #000000; margin-top: 0px; margin-buttom: 0px"))
...
else if (input$select1 == 5) {species_dataset<- "mmusculus_gene_ensembl"}
else if (input$select1 == 6) {species_dataset<- "rnorvegicus_gene_ensembl"}
else if (input$select1 == 7) {species_dataset<- "clfamiliaris_gene_ensembl"}
else if (input$select1 == 8) {species_dataset<- "dmelanogaster_gene_ensembl"}
else if (input$select1 == 9) {species_dataset<- "celegans_gene_ensembl"} 
ensembl <- useMart(biomart = "ENSEMBL_MART_ENSEMBL", dataset =  species_dataset, host = "useast.ensembl.org")})
...
if(input$ID_submit > 0){
  ...
  }else return(datasetInput())), options = list(scroller = TRUE, scrollY = 510, scrollX = TRUE, pageLength = 50), fillContainer = TRUE)
...
if(input$ID_submit > 0){
  ...
  }else return(datasetInput1())), options = list(scroller = TRUE, scrollY = 510, scrollX = TRUE, pageLength = 50), fillContainer = TRUE)
...
div(style = "display: inline-block; width: 150px;", selectInput("PCA_C_2", label = "", choices = c("PC1" = 1, "PC2"= 2, "PC3"= 3, "PC4" = 4), selected = 2)%>% 
    shinyInput_label_embed(
    shiny::icon("info") %>%
    bs_embed_tooltip(title = "PC1 vs PC2, PC2 vs PC3, PC3 vs PC4"))
)
...
downloadButton(inputId = "PCA_plot_trans", label = "Download transcriptomics", icon ("fas fa-download"), style = "color: #fff; background-color :#33B2FF")


downloadHandler(
filename = function(){
paste("PCA plot trans", input$select_PCA, sep = ".")
},
content = function(file){
...
downloadHandler(
filename = function(){
paste("PCA plot prot", input$select_PCA, sep = ".")
},
content = function(file){
ggsave(file, plot(Plot_PCA_2()), device = input$select_PCA, width = input$Download_PCA_input1, height = input$Download_PCA_input2, units = "cm", dpi = input$Download_PCA_input3)}
)
```
...
plot_t1 <- reactive({
if (length(data_list_2() == 8)){
PCA_T <- data_list_2()[, -c(1,3,4,6,7)]  
}

if (length(data_list_2() == 11)){
PCA_T <- data_list_2()[, -c(1,3,4,6,7,9,10)]  
}

if (length(data_list_2() == 14)){
PCA_T <- data_list_2()[, -c(1,3,4,6,7,9,10,12,13)]  
}
   })
...
validate(need(input$PCA_M != "", "1.Please select type of PCA in sidebar (\'Variable PCA or biplot PCA\')."))
...
output$PCA_plot_trans <- downloadHandler(
filename = function(){
...
output$PCA_plot_prot <- downloadHandler(
filename = function(){
paste("PCA plot", input$select_PCA, sep = ".")
},
content = function(file){
if(input$PCA_M > 0){ggsave(file, plot(Plot_PCA_2()), device = input$select_PCA, width = input$Download_PCA_input1, height = input$Download_PCA_input2, units = "cm", dpi = input$Download_PCA_input3)}
}
)
...
fluidRow(column (5, numericInput( inputId = "slider_1", label =  h5(strong("Pvalue/FDR cutoff ")), min = 0, max = 0.5, value = 0.001, step = 0.01, width = "100%")),

column(5, numericInput( inputId ="slider_2", label= h5(strong("Fold Change-cutoff")), min = 0, max = 15, value = 1.2 , step = 0.5, width = "100%")))
...
g1 <- ggplot(data = converted_data(), aes(x = converted_1[, 3], y = converted_1[, 2], col= converted_1$Differential_Expression)) + geom_point(size = 3)+ scale_x_continuous(breaks = seq(-20, 20, by = 2)) + scale_y_continuous(n.breaks = 10) + theme_bw() + xlab("logCPM") + ylab("logFC") + scale_color_manual(name = "Differential Expression", values = c("blue", "black","red")) + theme(text = element_text(size=20), plot.title = element_text(hjust = 0.5)) + ggtitle(input$text1)
}
})
output$plot_scatter <- renderPlot({
validate(
need(input$T_plot != "", "1. Please select \'Treatment\' in the sidebar (one treatment at a time only)."), 
need(input$checkbox != "", "2. Provide \'Pvalue\' (usual range is 0.001 to 0.05) or \'FDR cutoff\' (usual range is 0.01 to 0.05) and \'Fold change cutoff\' (usual range is 1 to 1.5).
3. Provide \'Title\'  to the scatter plot.
4. Please select checkbox \'Scatter plot transcriptomics\' in the sidebar.
5. Then select the image \'format\', adjust the \'dimension\' and \'resolution\'.
6. Click on \'Download\' button to download the image.")
)
...
need(input$T_plot != "", "1. Please select \'Treatment\' in the sidebar (one treatment at a time only)."), 
need(input$checkbox != "", "2. Provide \'Pvalue\' (usual range is 0.001 to 0.05) or \'FDR cutoff\' (usual range is 0.01 to 0.05) and \'Fold change cutoff\' (usual range is 1 to 1.5).
3. Provide \'Title\' to the volcano plot.
4. Please select checkbox \'Volcano plot transcriptomics\' in the sidebar.
5. Then select the image \'format\', adjust the \'dimension\' and \'resolution\'.
6. Click on \'Download\' button to download the image."))
...
validate(need(input$T_plot != "", "1. Please select \'Treatment\' in the sidebar (one treatment at a time only)"),   
need(input$checkbox != "", "2. Please select checkbox \'Table\' in the sidebar to show transcriptomics data with converted IDs.")) 
...
column (4, numericInput( inputId = "Numeric_input3",  label = h6("Pvalue/FDR"), value = 0, min = -20, max = 20, step = 0.01, width = '100%')))
...
tags$hr(style="height: 0px; border-width:1px; border-color: Black;")
...
checkboxGroupInput(inputId = "checkbox_3", label = h5(strong ("Histogram")), choices = list("One treatment", "Multi treatments"), inline = TRUE)
...
content = function(file){
if(input$checkbox_3 == "One treatment"){ggsave(file, plot(Histogram_plot_1()), device = tolower(input$select_format), width = input$Download_input1, height = input$Download_input2, dpi = input$Download_input3, units = "cm")}
else if(input$checkbox_3 == "Multi treatments"){ggsave(file, plot(Histogram_plot_all()), device = tolower(input$select_format), width = input$Download_input1, height = input$Download_input2, dpi = input$Download_input3, units = "cm")}
}
)
```
...
validate(need(input$T1_filter != "" , "1. Provide \'logFC cutoff\' (usual range is 1 to 1.5), \'logCPM\' (usual range is >= 3) and \'Pvalue\' (usual range is 0.001 to 0.05) or \'FDR cutoff\' (usual range is 0.01 to 0.05).
2. Select \'Omics Type\' (Transciptomics or Proteomics) in the sidebar.
3. Please select \'Treatment\' in the sidebar (one treatment at a time only).


\"To generate Venn diagram and Histogram statistical filtering input is required\"."))   
...
{Venn1.plot <- venn.diagram(x= list(A= A(), B= B(), C= E(), D= G()), fill = c("red", "blue", "green", "yellow"), cex = 2, cat.cex = 1.5, sub.cex = 5, height = 4000, width = 6000,  main.cex = 2, category.names = c("Up regulated 1", "Down regulated 1", "Up regulated 3", "Down regulated 3"), scale = TRUE, filename = NULL, main = input$text2, resolution = 500, units = "cm")
}
else if(all(c("Treatment-1", "Treatment-4") %in% input$T1_filter) == TRUE)
{Venn1.plot <- venn.diagram(x= list(A= A(), B= B(), C= H(), D= I()), fill = c("red", "blue", "green", "yellow"), cex = 2, cat.cex = 1.5, sub.cex = 5, height = 4000, width = 6000,  main.cex = 2, category.names = c("Up regulated 1", "Down regulated 1", "Up regulated 4", "Down regulated 4"), scale = TRUE, filename = NULL, main = input$text2, resolution = 500, units = "cm")}
})
...
if (all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_1(), B= A_2(), C= A_3(), D= A_4()), fill = c("red", "blue", "green", "yellow"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2, category.names = c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4"), height = 4000, width = 5000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}     
  
  
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_1(), B= A_2(), C= A_3()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-2", "Treatment-3"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}  

else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_1(), B= A_2(), C= A_4()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-2", "Treatment-4"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 

else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_2(), B= A_3(), C= A_4()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-2", "Treatment-3", "Treatment-4"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 

else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_1(), B= A_3(), C= A_4()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-3", "Treatment-4"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}    
   
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1_filter)){
Venn2.plot <-venn.diagram(x= list(A= A_1(), B= A_2()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-2"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}

else if(all(c("Treatment-1", "Treatment-3") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_1(), B= A_3()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-3"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 

else if(all(c("Treatment-1", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_1(), B= A_4()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-4"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}
  
else if(all(c("Treatment-2", "Treatment-3") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_2(), B= A_3()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-2", "Treatment-3"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 
  
else if(all(c("Treatment-2", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_2(), B= A_4()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-2", "Treatment-4"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}
  
else if(all(c("Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_3(), B= A_4()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-3", "Treatment-4"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}    
})
...
{Venn1.plot <- venn.diagram(x= list(A= A_1_1(), B= B_1(), C= E_1(), D= G_1()), fill = c("red", "blue", "green", "yellow"), cex = 2, cat.cex = 1.5, sub.cex = 5, height = 4000, width = 6000,  main.cex = 2, category.names = c("Up regulated 1", "Down regulated 1", "Up regulated 3", "Down regulated 3"), scale = TRUE, filename = NULL, main = input$text2, resolution = 500, units = "cm")
}
else if(all(c("Treatment-1", "Treatment-4") %in% input$T1_filter) == TRUE)
{Venn1.plot <- venn.diagram(x= list(A= A_1_1(), B= B_1(), C= H_1(), D= I_1()), fill = c("red", "blue", "green", "yellow"), cex = 2, cat.cex = 1.5, sub.cex = 5, height = 4000, width = 6000,  main.cex = 2, category.names = c("Up regulated 1", "Down regulated 1", "Up regulated 4", "Down regulated 4"), scale = TRUE, filename = NULL, main = input$text2, resolution = 500, units = "cm")}
})
...
if (all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= AA_1(), B= A_2_1(), C= A_3_1(), D= A_4_1()), fill = c("red", "blue", "green", "yellow"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2, category.names = c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4"), height = 4000, width = 5000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}     
  
  
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= AA_1(), B= A_2_1(), C= A_3_1()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-2", "Treatment-3"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}  

else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= AA_1(), B= A_2_1(), C= A_4_1()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-2", "Treatment-4"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 

else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_2_1(), B= A_3_1(), C= A_4_1()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-2", "Treatment-3", "Treatment-4"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 

else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= AA_1(), B= A_3_1(), C= A_4_1()), fill = c("red", "blue", "green"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-3", "Treatment-4"), height = 4000, width = 3000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}   
   
   
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1_filter)){
Venn2.plot <-venn.diagram(x= list(A= AA_1(), B= A_2_1()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-2"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}

else if(all(c("Treatment-1", "Treatment-3") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= AA_1(), B= A_3_1()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-3"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 

else if(all(c("Treatment-1", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= AA_1(), B= A_4_1()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-1", "Treatment-4"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}
  
else if(all(c("Treatment-2", "Treatment-3") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_2_1(), B= A_3_1()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-2", "Treatment-3"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
} 
  
else if(all(c("Treatment-2", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_2_1(), B= A_4_1()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-2", "Treatment-4"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}
  
else if(all(c("Treatment-3", "Treatment-4") %in% input$T1_filter)){
Venn2.plot <- venn.diagram(x= list(A= A_3_1(), B= A_4_1()), fill = c("red", "blue"), cex = 2, cat.cex = 1.5, sub.cex = 5, main.cex = 2,  category.names = c("Treatment-3", "Treatment-4"), height = 4000, width = 2000, scale = TRUE, main = input$text2, resolution = 500, units = "cm", filename = NULL)
}    
})
...
validate(need(input$T1_filter != "" , ""),
need(input$checkbox_2 != "", "To draw \'Split Venn diagram\' (with up/down regulated candidates).
1. Please select treatments in pairs.
2. Then select checkbox \'Split into Up and Down-regulated\' in the sidebar.
OR
To draw \'Venn diagram\'.
1. Select treatments in pairs.
2. Then select checkbox \'VennDiagram\' in the sidebar.

1. Select the image \'Format\', adjust the \'dimension\' and \'resolution\'.
2. Provide \'Title\' to the Venn diagram using \'Title\' in the sidebar. 
3. Click on first \'Download\' button to download the image.")
) 
...
if("Transcriptomics" %in% input$select_omics){
A_1 <- as.data.frame(A())
...
rbind.data.frame(df_histo_2, df_histo_2_2)
}
else if(input$select_omics == "Proteomics"){
...
rbind.data.frame(df_histo_3, df_histo_3_3)
}
else if(input$select_omics == "Proteomics"){
...
rbind.data.frame(df_histo_4, df_histo_4_4)
}
else if(input$select_omics == "Proteomics"){
...
all_histo_12 <- reactive({rbind.data.frame(as.data.frame(all_histo_1()), as.data.frame(all_histo_2_1()))})

all_histo_13 <- reactive({rbind.data.frame(as.data.frame(all_histo_1()), as.data.frame(all_histo_3_1()))})

all_histo_14 <- reactive({rbind.data.frame(as.data.frame(all_histo_1()), as.data.frame(all_histo_4_1()))})

all_histo_23 <- reactive({rbind.data.frame(as.data.frame(all_histo_2_1()), as.data.frame(all_histo_3_1()))})

all_histo_24 <- reactive({rbind.data.frame(as.data.frame(all_histo_2_1()), as.data.frame(all_histo_4_1()))})

all_histo_34 <- reactive({rbind.data.frame(as.data.frame(all_histo_3_1()), as.data.frame(all_histo_4_1()))})

all_histo_24 <- reactive({rbind.data.frame(as.data.frame(all_histo_2_1()), as.data.frame(all_histo_4_1()))})

all_histo_123 <- reactive({rbind.data.frame(as.data.frame(all_histo_12()), as.data.frame(all_histo_3_1()))})

all_histo_134 <- reactive({rbind.data.frame(as.data.frame(all_histo_13()), as.data.frame(all_histo_4_1()))})

all_histo_234 <- reactive({rbind.data.frame(as.data.frame(all_histo_23()), as.data.frame(all_histo_4_1()))})

all_histo_124 <- reactive({rbind.data.frame(as.data.frame(all_histo_12()), as.data.frame(all_histo_4_1()))})

all_histo_1234 <- reactive({rbind.data.frame(as.data.frame(all_histo_123()), as.data.frame(all_histo_4_1()))})
...
if(all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1_filter))
{H_plot4 <- ggplot(all_histo_1234(), aes(x= all_histo_1234()[, 1], y= all_histo_1234()[, 3], fill= all_histo_1234()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_1234()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}  

 else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1_filter))
{H_plot3 <- ggplot(all_histo_123(), aes(x= all_histo_123()[, 1], y= all_histo_123()[, 3], fill= all_histo_123()[, 2])) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_123()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}  

else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1_filter))
{H_plot3 <- ggplot(all_histo_134(), aes(x= all_histo_134()[, 1], y= all_histo_134()[, 3], fill= all_histo_134()[, 2])) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_134()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}  
  
else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1_filter))
{H_plot3 <- ggplot(all_histo_234(), aes(x= all_histo_234()[, 1], y= all_histo_234()[, 3], fill= all_histo_234()[, 2])) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_234()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}

else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1_filter))
{H_plot3 <- ggplot(all_histo_124(), aes(x= all_histo_124()[, 1], y= all_histo_124()[, 3], fill= all_histo_124()[, 2])) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_124()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}
          
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1_filter))
{H_plot2 <- ggplot(all_histo_12(), aes(x= all_histo_12()[, 1], y= all_histo_12()[, 3], fill= all_histo_12()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_12()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}

else if(all(c("Treatment-1", "Treatment-3") %in% input$T1_filter))
{H_plot2 <- ggplot(all_histo_13(), aes(x= all_histo_13()[, 1], y= all_histo_13()[, 3], fill= all_histo_13()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_13()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}  

else if(all(c("Treatment-2", "Treatment-3") %in% input$T1_filter))
{H_plot2 <- ggplot(all_histo_23(), aes(x= all_histo_23()[, 1], y= all_histo_23()[, 3], fill= all_histo_23()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_23()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}  

else if(all(c("Treatment-2", "Treatment-4") %in% input$T1_filter))
{H_plot2 <- ggplot(all_histo_24(), aes(x= all_histo_24()[, 1], y= all_histo_24()[, 3], fill= all_histo_24()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_24()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}   

else if(all(c("Treatment-1", "Treatment-4") %in% input$T1_filter))
{H_plot2 <- ggplot(all_histo_14(), aes(x= all_histo_14()[, 1], y= all_histo_14()[, 3], fill= all_histo_14()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_14()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}   

else if(all(c("Treatment-3", "Treatment-4") %in% input$T1_filter))
{H_plot2 <- ggplot(all_histo_34(), aes(x= all_histo_34()[, 1], y= all_histo_34()[, 3], fill= all_histo_34()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_34()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}  
  
        
else if(input$T1_filter == "Treatment-1")
{H_plot1 <- ggplot(all_histo_1(), aes(x= all_histo_1()[, 1], y= all_histo_1()[, 3], fill= all_histo_1()[, 2] )) + geom_bar(stat="identity", position= "dodge") + ggtitle(input$text2) + theme_bw() + xlab("Number of genes") + ylab("") + geom_text(aes(label=all_histo_1()[, 1]), color="Black", position = position_dodge(0.9), size= 6) + theme(text = element_text(size=20))+ labs(fill = "regulation")
}
...
validate(
need(input$T1_filter != "" , ""),
need(input$checkbox_3 != "", "To draw \'One treatment\' histogram.
1. Please select \'Treatment\' in the sidebar (one at a time only).
2. Select checkbox \'One treatment\' in the sidebar.
OR
To draw \'Multi treatments\' histogram (with up/down regulated candidates). 
1. Select \'Treatments\' in the sidebar.
2. Select checkbox \'Multi treatments\' histogram in the sidebar (below Histogram).

1. Select the image \'Format\', adjust the \'dimension\' and \'resolution\'.
2. Provide title to the Venn diagram using \'Title\' in the sidebar. 
3. Click on second \'Download\' button to download the image.")
)  
if(input$checkbox_3 == "One treatment"){
...
else if (input$checkbox_3 == "Multi treatments"){
...
checkboxGroupInput(inputId = "select_GO", label = h5(strong("Gene ontology classes")), choices = list("GO Biological Process (BP)" = 1, "GO Molecular Function (MF)" = 2, "GO Cellular Component (CC)"= 3), inline = FALSE)
...
div(style = "display: inline-block; width: 150px;", numericInput(inputId = "cutoff",  label = h6("Pvalue cutoff"), value = 0, min = 0, max = 20, step = 0.01 , width = '100%')%>% 
    shinyInput_label_embed(
    shiny::icon("info") %>%
    bs_embed_tooltip(title = "Use for both ORA and GSEA")))
...
div(style = "display: inline-block; width: 150px;",  numericInput(inputId = "qcutoff",  label = h6("q-value cutoff ORA"), value = 0, min = 0, max = 20, step = 0.01, width = '100%')%>% 
    shinyInput_label_embed(
    shiny::icon("info") %>%
    bs_embed_tooltip(title = "Show the GO terms that are lesser than q value cutoff")))
...
div(h5(tags$b("Once the result is ready, select only one row (one enriched GO term at a time) in all the enriched result for heatmap visualization in next tab (\'GO heatmaps\')."), style = "display: inline;color: #000000;"))
```
...
else if (input$select1 == 5) {org.Mm.eg.db}
else if (input$select1 == 6) {org.Rn.eg.db}
else if (input$select1 == 7) {org.Cf.eg.db}
else if (input$select1 == 8) {org.Dm.eg.db}
else if (input$select1 == 9) {org.Ce.eg.db}
})
...
validate(need(input$select_GO != "", ""),
need(input$checkbox_GSE_GO != "", "1. Please select \'Omics Type\' in the sidebar.
2. Select only one \'Gene ontology class\' at a time (BP or MF or CC).
3. Provide \'Pvalue cutoff\' (usual range is 0.001 to 0.05).
4. Provide \'q-value cutoff for ORA\' (usual range is 0.01 to 0.05).
5. Select only one \'pAdjustment Method\' at a time (BH method is recommended).
6. Select enrichment \'GO ORA or GO GSEA\' gene ontology analysis (only one at a time)."),
need(input$submit_GO != "", "7. Click on \'Go\' button to perform gene ontology enrichment analysis.
8. Click on window panel \'Treatment\' for the results.
9. To download click on checkbox \'Treatment-1, 2, 3 or 4\'(one at a time only).
10. Click on \'Download\' button to download the enrichment result in csv format.")
)  
  
GO_final_table_1()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'),fillContainer = TRUE)
...
validate(
need(input$select_GO != "", ""),
need(input$checkbox_GSE_GO != "", "1. Treatment-2 will use the same \'Omics Type\', \'Gene ontology class\', \'P-value cutoff\', \'q-value cutoff ORA\', \'pAdjusment Method\', and \'enrichment method\' provided in the sidebar.
2. Click on \'Go\' button to perform gene ontology enrichment analysis (if not automatically executed)."))  

  
GO_final_table_2()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'), fillContainer = TRUE)
...
validate(
need(input$select_GO != "", ""),
need(input$checkbox_GSE_GO != "", "1. Treatment-3 will use the same \'Omics Type\', \'Gene ontology class\', \'P-value cutoff\', \'q-value cutoff ORA\', \'pAdjusment Method\', and \'Enrichment analysis method\' provided in the sidebar.
2. Click on \'Go\' button to perform gene ontology enrichment analysis (if not automatically executed)."))  

GO_final_table_3()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'), fillContainer = TRUE)
...
validate(
need(input$select_GO != "", ""),
need(input$checkbox_GSE_GO != "", "1. Treatment-4 will use the same \'Omics Type\', \'Gene ontology class\', \'P-value cutoff\', \'q-value cutoff ORA\', \'pAdjusment Method\', and \'enrichment method\' provided in the sidebar.
2. Click on \'Go\' button to perform gene ontology enrichment analysis (if not automatically executed)."))  
GO_final_table_4()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'), fillContainer = TRUE)
...
if (c("Treatment-1" %in% input$heatmapT1) &&  ("GO GSEA" %in% input$checkbox_heatmap_GO)){
...
heatmap_f4 <- as.data.frame(replace(h_plot_gse_2_2, is.na(h_plot_gse_2_2), 0))
}
})

heatmap_gse_3 <- reactive({
if (c("Treatment-3" %in% input$heatmapT1) &&  ("GO GSEA" %in% input$checkbox_heatmap_GO)){   
...
heatmap_gse_4 <- reactive({
if (c("Treatment-4" %in% input$heatmapT1) &&  ("GO GSEA" %in% input$checkbox_heatmap_GO)){  
...
if(all(c("Treatment-1","Treatment-2", "Treatment-3","Treatment-4") %in% input$heatmapT1)) {

heatmap_f3_1 <- as.data.frame(heatmap_gse_1())
heatmap_f3_2 <- as.data.frame(heatmap_gse_2())
heatmap_f3_3 <- as.data.frame(heatmap_gse_3())
heatmap_f3_4 <- as.data.frame(heatmap_gse_4())  
  
heatmap_f_m1 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))

heatmap_f_m2 <- merge(heatmap_f_m1, heatmap_f3_3, by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))

heatmap_f_m31 <- merge(heatmap_f_m2, heatmap_f3_4, by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))

heatmap_f_m3 <- as.data.frame(replace(heatmap_f_m31, is.na(heatmap_f_m31), 0))

heatmap_f2 <- heatmap_f_m3[!duplicated(heatmap_f_m3[["external_gene_name"]]), ]
heatmap_f2
heatmap_f_m3[heatmap_f_m3 == " "] <- NA
as.data.frame(heatmap_f_m3)
...
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-1","Treatment-2", "Treatment-3","Treatment-4"), col = rlm, lty= 1,lwd = 8,bty = "n") 
}  

else if(all(c("Treatment-1","Treatment-2", "Treatment-3") %in% input$heatmapT1)) {
...
heatmap_f_m3 <- as.data.frame(replace(heatmap_f_m2, is.na(heatmap_f_m2), 0))
...
else if(all(c("Treatment-1","Treatment-3", "Treatment-4") %in% input$heatmapT1)) {

heatmap_f3_1 <- as.data.frame(heatmap_gse_1())
heatmap_f3_3 <- as.data.frame(heatmap_gse_3())
heatmap_f3_4 <- as.data.frame(heatmap_gse_4())
  
heatmap_f_m1 <- merge(heatmap_f3_1, heatmap_f3_3, by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))

heatmap_f_m2 <- merge(heatmap_f_m1, heatmap_f3_4, by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))
...
else if(all(c("Treatment-1","Treatment-2", "Treatment-4") %in% input$heatmapT1)) {
...
rlm <- rainbow(ncol(heatmap_22))
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-1","Treatment-2", "Treatment-4"), col = rlm, lty= 1,lwd = 8,bty = "n") 
}      
  

else if(all(c("Treatment-2","Treatment-3", "Treatment-4") %in% input$heatmapT1)) {

heatmap_f3_2 <- as.data.frame(heatmap_gse_2())
heatmap_f3_3 <- as.data.frame(heatmap_gse_3())
heatmap_f3_4 <- as.data.frame(heatmap_gse_4())
  
heatmap_f_m1 <- merge(heatmap_f3_2, heatmap_f3_3, by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_22))
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-2","Treatment-3", "Treatment-4"), col = rlm, lty= 1,lwd = 8,bty = "n") 
}  
    

else if (all(c("Treatment-1", "Treatment-2") %in% input$heatmapT1)){

heatmap_f12 <- merge(heatmap_gse_1(), heatmap_gse_2(), by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))

heatmap_f1 <- as.data.frame(replace(heatmap_f12, is.na(heatmap_f12), 0))
...
heatmap_f12 <- merge(heatmap_gse_1(), heatmap_gse_3(), by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_22))
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-1", "Treatment-3"), col = rlm, lty= 1,lwd = 8,bty = "n")
}

else if (all(c("Treatment-2", "Treatment-3") %in% input$heatmapT1)){

heatmap_f12 <- merge(heatmap_gse_2(), heatmap_gse_3(), by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_22))
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-2", "Treatment-3"), col = rlm, lty= 1,lwd = 8,bty = "n")
}
 
else if (all(c("Treatment-2", "Treatment-4") %in% input$heatmapT1)){
  
heatmap_f3_2 <- as.data.frame(heatmap_gse_2())
heatmap_f3_4 <- as.data.frame(heatmap_gse_4())
    
heatmap_f12 <- merge(heatmap_f3_2, heatmap_f3_4, by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_22))
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-2", "Treatment-4"), col = rlm, lty= 1,lwd = 8,bty = "n")
}     
  
  
else if (all(c("Treatment-1", "Treatment-4") %in% input$heatmapT1)){
heatmap_f12 <- merge(heatmap_gse_1(), heatmap_gse_4(), by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_22))
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-1", "Treatment-4"), col = rlm, lty= 1,lwd = 8,bty = "n")
}

else if (all(c("Treatment-3", "Treatment-4") %in% input$heatmapT1)){
heatmap_f12 <- merge(heatmap_gse_3(), heatmap_gse_4(), by.y = "external_gene_name", by.x = "external_gene_name", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_22))
heatmap.2(heatmap_22, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none",  margins = c(3,22), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,) 
legend("topright", legend = c("Treatment-3", "Treatment-4"), col = rlm, lty= 1,lwd = 8,bty = "n")
}
})


heatmap_ora_1 <- reactive({
if (c("Treatment-1" %in% input$heatmapT1) &&  ("GO ORA" %in% input$checkbox_heatmap_GO)){  
...
heatmap_ora_2 <- reactive({
if (c("Treatment-2" %in% input$heatmapT1) &&  ("GO ORA" %in% input$checkbox_heatmap_GO)){
...
heatmap_f3 <- as.data.frame(replace(h_plot_data_2_2, is.na(h_plot_data_2_2), 0))
}
})
heatmap_ora_3 <- reactive({
if (c("Treatment-3" %in% input$heatmapT1) &&  ("GO ORA" %in% input$checkbox_heatmap_GO)){  
...
heatmap_f3 <- as.data.frame(replace(h_plot_data_3_3, is.na(h_plot_data_3_3), 0))
}
})
heatmap_ora_4 <- reactive({
if (c("Treatment-4" %in% input$heatmapT1) &&  ("GO ORA" %in% input$checkbox_heatmap_GO)){  
...
heatmap_f3 <- as.data.frame(replace(h_plot_data_4_4, is.na(h_plot_data_4_4), 0))
}
})
...
if (all(c("Treatment-1","Treatment-2", "Treatment-3","Treatment-4") %in% input$heatmapT1)) {
heatmap_f3_1 <- as.data.frame(heatmap_ora_1())
heatmap_f3_2 <- as.data.frame(heatmap_ora_2())
heatmap_f3_3 <- as.data.frame(heatmap_ora_3())
heatmap_f3_4 <- as.data.frame(heatmap_ora_4())

heatmap_f12 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f23 <- merge(heatmap_f12, heatmap_f3_3, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f1234 <- merge(heatmap_f23, heatmap_f3_4, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))

heatmap_f3 <- as.data.frame(replace(heatmap_f1234, is.na(heatmap_f1234), 0))
...
heatmap_f3_3 <- as.data.frame(heatmap_ora_3())  
heatmap_f12 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f123 <- merge(heatmap_f12, heatmap_f3_3, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))

heatmap_f3 <- as.data.frame(replace(heatmap_f123, is.na(heatmap_f123), 0))

heatmap_f5 <- column_to_rownames(.data = heatmap_f3, var = "geneID")
heatmap_ora_1_1 <- as.matrix(heatmap_f5)
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(2,20), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,)
...
heatmap_f3_1 <- as.data.frame(heatmap_ora_1())
heatmap_f3_2 <- as.data.frame(heatmap_ora_2())
heatmap_f3_3 <- as.data.frame(heatmap_ora_4())  
heatmap_f12 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f123 <- merge(heatmap_f12, heatmap_f3_3, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(2,20), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,)
legend("topright", legend = c("Treatment-1", "Treatment-2", "Treatment-4"), col = rlm, lty= 1,lwd = 5,bty = "n")
}  
  
  
else if (all(c("Treatment-1","Treatment-3", "Treatment-4") %in% input$heatmapT1)) {
heatmap_f3_1 <- as.data.frame(heatmap_ora_1())
heatmap_f3_2 <- as.data.frame(heatmap_ora_3())
heatmap_f3_3 <- as.data.frame(heatmap_ora_4())  
heatmap_f12 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f123 <- merge(heatmap_f12, heatmap_f3_3, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(2,20), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,)
legend("topright", legend = c("Treatment-1", "Treatment-3", "Treatment-4"), col = rlm, lty= 1,lwd = 5,bty = "n")
}

else if (all(c("Treatment-2","Treatment-3", "Treatment-4") %in% input$heatmapT1)) {
heatmap_f3_1 <- as.data.frame(heatmap_ora_2())
heatmap_f3_2 <- as.data.frame(heatmap_ora_3())
heatmap_f3_3 <- as.data.frame(heatmap_ora_4())  
heatmap_f12 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f123 <- merge(heatmap_f12, heatmap_f3_3, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
...
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(2,20), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0,)
legend("topright", legend = c("Treatment-2", "Treatment-3", "Treatment-4"), col = rlm, lty= 1,lwd = 5,bty = "n")
}  
...
heatmap_f3_1 <- as.data.frame(heatmap_ora_1())
heatmap_f3_2 <- as.data.frame(heatmap_ora_2())
heatmap_f3_3 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f3 <- as.data.frame(replace(heatmap_f3_3, is.na(heatmap_f3_3), 0))

heatmap_f5 <- column_to_rownames(.data = heatmap_f3, var = "geneID") %>% unique()
heatmap_ora_1_1 <- as.matrix(heatmap_f5)
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(3,30), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0) 
...
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(3,30), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0) 
legend("topright", legend = c("Treatment-1", "Treatment-3"), col = rlm, lty= 1,lwd = 5, bty = "n")
}  

else if (all(c("Treatment-1", "Treatment-4") %in% input$heatmapT1)){
heatmap_f3_1 <- as.data.frame(heatmap_ora_1())
heatmap_f3_2 <- as.data.frame(heatmap_ora_4())
heatmap_f3_3 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f3 <- as.data.frame(replace(heatmap_f3_3, is.na(heatmap_f3_3), 0))
...
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(3,30), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0) 
legend("topright", legend = c("Treatment-1", "Treatment-4"), col = rlm, lty= 1,lwd = 5, bty = "n")
}    
...
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(3,30), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0) 
legend("topright", legend = c("Treatment-2", "Treatment-3"), col = rlm, lty= 1,lwd = 5, bty = "n")
}   
...
rlm <- rainbow(ncol(heatmap_ora_1_1))
heatmap.2(heatmap_ora_1_1, ColSideColors = rlm, main = input$heat_text, keysize = input$heatmap_input3, key.title = NA, key.xlab = NA, key.ylab = NA, dendrogram = "none", col = bluered(255), Rowv = FALSE, Colv = FALSE, key = TRUE, trace = "none", margins = c(3,30), cexCol = input$heatmap_input1, cexRow = input$heatmap_input2, srtCol = 0, adjCol = c(0.5, 0), offsetRow = 0) 
legend("topright", legend = c("Treatment-2", "Treatment-4"), col = rlm, lty= 1,lwd = 5, bty = "n")
}   
  
else if (all(c("Treatment-3", "Treatment-4") %in% input$heatmapT1)){
heatmap_f3_1 <- as.data.frame(heatmap_ora_3())
heatmap_f3_2 <- as.data.frame(heatmap_ora_4())
heatmap_f3_3 <- merge(heatmap_f3_1, heatmap_f3_2, by.y = "geneID", by.x = "geneID", all = TRUE, suffixes = c("", ""))
heatmap_f3 <- as.data.frame(replace(heatmap_f3_3, is.na(heatmap_f3_3), 0))
...
if (c("Treatment-1" %in% input$heatmapT1) && ("GO GSEA" %in% input$checkbox_heatmap_GO))
{heatmap_gse_1()}
else if (c("Treatment-2" %in% input$heatmapT1) && ("GO GSEA" %in% input$checkbox_heatmap_GO))
{heatmap_gse_2()}
...
{heatmap_gse_4()} 
else if (c("Treatment-1" %in% input$heatmapT1) &&  ("GO ORA" %in% input$checkbox_heatmap_GO))
{heatmap_ora_1()}   
...
validate(need(input$checkbox_heatmap_GO != "", "1. Please select checkbox \'GO ORA\' or \'GO GSEA\' (one at a time only) below heatmap visualization."),
need(input$heatmapT1 != "", "2. Select \'Treatments\' to generate hetamap (minimum two treatments are required).
3. Ajust the \'column text\', \'row text\' size and \'color key\' size according to the display.
4. Provide \'Title\' to the generated heatmap.
5. Then select the image \'Format\', adjust the \'dimension\' and \'resolution\'.
6. Click on first \'Download\' button to download the heatmap."))  
...
validate(
need(input$checkbox_heatmap_GO != "", "1. Please select checkbox \'GO ORA\' or \'GO GSEA\' (one at a time only) for the construction of word cloud."),
need(input$heatmapT1 != "", "2. Select \'Treatments\' (only one at a time)."),
need(input$checkbox_4 != "", "3. Select the image \'Format\', adjust the \'dimension\' and \'resolution\' for word Cloud.
4. Select checkbox \'Word Cloud'\ and provide the number of \'maximum words\' to display in the Word Cloud.
5. To download the word cloud select click on the second download button."))
...
```{r, echo=FALSE}
 output$Table_output_3 <- DT::renderDataTable({
validate(need(input$checkbox_heatmap_GO != "", "1. Please select checkbox \'GO ORA\' or \'GO GSEA\' (one at a time only)."),
need(input$heatmapT1 != "", "2. Select checkbox \'Treatments\' (only one at a time) in the sidebar to view each treatment heatmap table."))
   
   final_gse_table()}, options = list(scroller = TRUE, scrollY = 510, scrollX = TRUE, pageLength = 25))
...
div(style = "margin-top: 0px; margin-buttom: 0px;", checkboxGroupInput(inputId = "checkbox_KEGG", label = h5(strong("Pathway analysis type")), choices = list("Over-Representation analysis (ORA)", "Gene Set Enrichment Analysis (GSEA)", "ReactomePA (Human)"), inline = FALSE))


div(style = "display: inline-block; width: 150px;", numericInput(inputId = "cutoff_KEGG",  label = h6("Pvalue cutoff"), value = 0, min = 0, max = 20, step = 0.01 , width = '100%')%>% 
    shinyInput_label_embed(
    shiny::icon("info") %>%
    bs_embed_tooltip(title = "Use for both enrichKEGG, GSEKEGG and Reactome")))

div(style = "display: inline-block; width: 150px;",  numericInput(inputId = "qcutoff_KEGG",  label = h6("q-value cutoff ORA"), value = 0, min = 0, max = 20, step = 0.01, width = '100%')%>% 
    shinyInput_label_embed(
    shiny::icon("info") %>%
    bs_embed_tooltip(title = "Show the enriched pathway that are lesser than q value cutoff (only for enrichKEGG and Reactome")))
...
div(style = "display: inline-block;",checkboxGroupInput(inputId = "checkbox_KEGG_1", label = h5(strong("Network Topology Analysis (NTA)")), choices = c("NTA"), width = '100%'))

selectInput(inputId = "Pathway_database", label = h6(strong("Databases for NTA")), choices = c("biocarta", "panther", "nci", "pharmgkb"))

tags$hr(style="height: 0px; border-width:1px; border-color: Black;")

checkboxGroupInput(inputId = "checkbox_KEGG_2", label = h5(strong("STRING")), choices = c("String"))
...
checkboxGroupInput(inputId = "pathway_T1", label = h6(strong("To downaload table of enrichment result")), choices = list("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4"),inline = TRUE)
...
div(h5(tags$b("Once the result is ready, select only one row and same enriched pathway in all the enriched result for pathway visualization in next tab (\'Enriched pathway visualization\')."), style = "display: inline;color: #000000;"))
...
else if (input$select1 == 5) {org_kegg_1 <- "mmu" }
else if (input$select1 == 6) {org_kegg_1 <- "rno" }
else if (input$select1 == 7) {org_kegg_1 <- "cfa" }
else if (input$select1 == 8) {org_kegg_1 <- "cel" }
else if (input$select1 == 9) {org_kegg_1 <- "dme" }
})

#Code for ORA KEGG data
KEGG_table_ora_1 <- reactive({

if(c("Over-Representation analysis (ORA)" %in% input$checkbox_KEGG) && ("Transcriptomics" %in% input$select_omics_GO_path)){
df_3 <- as.data.frame(Filtertable1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge <- merge(df_3, df_with_Etzgene_1, by.x = "ENSEMBLGENE", by.y = "ensembl_gene_id")
KEGG_output_result <- clusterProfiler:: enrichKEGG(gene = as.vector(GO_merge$entrezgene_id), organism = keg_spec_1(), pvalueCutoff = input$cutoff_KEGG, qvalueCutoff = input$qcutoff_KEGG,  pAdjustMethod = adjust_method_1(), minGSSize = 1)}

else if(c("Over-Representation analysis (ORA)" %in% input$checkbox_KEGG) && ("Proteomics" %in% input$select_omics_GO_path)){
df_3 <- df_3_1()
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge <- merge(df_3, df_with_Etzgene_1, by.x = "UniProt ID", by.y = "uniprotswissprot")
KEGG_output_result_pro <- clusterProfiler:: enrichKEGG(gene = as.vector(GO_merge$entrezgene_id), organism = keg_spec_1(), pvalueCutoff = input$cutoff_KEGG, qvalueCutoff = input$qcutoff_KEGG,  pAdjustMethod = adjust_method_1(), minGSSize = 1)}
  })

#code for GSEA KEGG data
KEGG_table_gse_1 <- reactive({
if (c("Gene Set Enrichment Analysis (GSEA)" %in% input$checkbox_KEGG) && ("Transcriptomics" %in% input$select_omics_GO_path)){
df_3 <- as.data.frame(Filtertable1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
...
else if(c("Gene Set Enrichment Analysis (GSEA)" %in% input$checkbox_KEGG) && ("Proteomics" %in% input$select_omics_GO_path)){ df_3 <- as.data.frame(df_3_1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
...
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge <- merge(df_3, df_with_Etzgene_1, by.x = "ENSEMBLGENE", by.y = "ensembl_gene_id")
KEGG_output_result <- enrichPathway(gene = as.vector(GO_merge$entrezgene_id), organism = "human", pvalueCutoff = input$cutoff_KEGG, qvalueCutoff = input$qcutoff_KEGG, pAdjustMethod = adjust_method_1(), minGSSize = 1)}

else if(c("ReactomePA (Human)" %in% input$checkbox_KEGG) && ("Proteomics" %in% input$select_omics_GO_path)){
df_3 <- as.data.frame(df_3_1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge <- merge(df_3, df_with_Etzgene_1, by.x = "UniProt ID", by.y = "uniprotswissprot")
KEGG_output_result_pro <- enrichPathway(gene = as.vector(GO_merge$entrezgene_id), organism = "human", pvalueCutoff = input$cutoff_KEGG, qvalueCutoff = input$qcutoff_KEGG, pAdjustMethod = adjust_method_1(), minGSSize = 1) }  
  })
...
if(c("NTA" %in% input$checkbox_KEGG_1) && ("Transcriptomics" %in% input$select_omics_GO_path)){
df_3 <- as.data.frame(Filtertable1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
...
SPIA_database <- graphite::pathways("hsapiens", input$Pathway_database)
...
else if(c("NTA" %in% input$checkbox_KEGG_1) && ("Proteomics" %in% input$select_omics_GO_path)){ 
df_3 <- as.data.frame(df_3_1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
...
if(c(1 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 9606, score_threshold = 200, input_directory = "")
string_db
}
  
else if(c(2 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 9031, score_threshold = 200, input_directory = "")
string_db
} 

else if(c(3 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 9823, score_threshold = 200, input_directory = "")
string_db
}

else if(c(4 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 9913, score_threshold = 200, input_directory = "")
string_db
}

else if(c(5 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2E)) {
string_db <- STRINGdb$new(version = "11", species = 10090, score_threshold = 200, input_directory = "")
string_db
}  

else if(c(6 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 10114, score_threshold = 200, input_directory = "")
string_db
}  
  
else if(c(7 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 9615, score_threshold = 200, input_directory = "")
string_db
}  
  
else if(c(8 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 7227, score_threshold = 200, input_directory = "")
string_db
}  
  
else if(c(9 %in% input$select1) && ("String" %in% input$checkbox_KEGG_2)) {
string_db <- STRINGdb$new(version = "11", species = 6239, score_threshold = 200, input_directory = "")
string_db
}    
   })
...
if(c("String" %in% input$checkbox_KEGG_2) && ("Transcriptomics" %in% input$select_omics_GO_path)){  
df_3 <- as.data.frame(Filtertable1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge <- merge(df_3, df_with_Etzgene_1, by.x = "ENSEMBLGENE", by.y = "ensembl_gene_id")
example1_mapped <- string_table_1()$map(GO_merge, "external_gene_name", removeUnmappedRows = TRUE)
}
else if(c("String" %in% input$checkbox_KEGG_2) && ("Proteomics" %in% input$select_omics_GO_path)){
df_3 <- as.data.frame(df_3_1())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge <- merge(df_3, df_with_Etzgene_1, by.x = "UniProt ID", by.y = "uniprotswissprot")
example1_mapped <- string_table_1()$map(GO_merge, "external_gene_name", removeUnmappedRows = TRUE)
...
incProgress(1/15)}
KEGG_1 <- isolate(
if ("Over-Representation analysis (ORA)" %in% input$checkbox_KEGG){
as.data.frame(KEGG_table_ora_1()@result)}
else if ("Gene Set Enrichment Analysis (GSEA)" %in% input$checkbox_KEGG){
as.data.frame(KEGG_table_gse_1()@result)}
else if ("ReactomePA (Human)" %in% input$checkbox_KEGG ){
as.data.frame(reactome_table_1()@result)  
}
else if ("NTA" %in% input$checkbox_KEGG_1){
as.data.frame(network_topology_1())  
}
else if ("String" %in% input$checkbox_KEGG_2){
...
validate(need(input$submit_KEGG != 0,
"1. Please select checkbox \'Omics Type\' in the sidebar (only one at a time).
2. Select \'Pathway analysis type \' (only one at a time).
3. Provide \'Pvalue cutoff\' for \'ORA\', \'GSEA\' and \'ReactomePA\' (usual range is 0.001 to 0.05).
4. Provide \'q-value cutoff\' for \'ORA\' and \'ReactomePA\' (usual range is 0.01 to 0.05).
5. Select \'pAdjustment method\' one at a time only (BH method is recommended).
6. Click \'Go\' button to perform  \'pathway enrichment analysis\'.
OR
1. Select checkbox \'Network Topology Analysis (NTA) \' (only one at a time).
2. Then select database to perform \'NTA\' (one at a time only), available only for human data.
3. Click \'Go\' button to perform  \'Network Topology Analysis (NTA)\'.
OR
1. Select checkbox \'STRING \' (only one at a time).
2. Click \'Go\' button to perform  \'STRING\'.

To download the result from each enrichment analysis.
1. Select checkbox \'treatments\' to download the result of each treatment (one at a time only).
2. Click on \'download\' button to download result in csv format."))


KEGG_final_table_1()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'), fillContainer = TRUE )
...
else if(c("Gene Set Enrichment Analysis (GSEA)" %in% input$checkbox_KEGG) && ("Proteomics" %in% input$select_omics_GO_path)){
'this is the uploaded data that will be used for the fetching of logfc value of the gene' 
...
KEGG_output_result_2 <- clusterProfiler:: enrichKEGG(gene = as.vector(GO_merge_2$entrezgene_id), organism = keg_spec_1(), pvalueCutoff = input$cutoff_KEGG, qvalueCutoff = input$qcutoff_KEGG, pAdjustMethod = adjust_method_1(), minGSSize = 1)}
...
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge <- merge(df_3, df_with_Etzgene_1, by.x = "UniProt ID", by.y = "uniprotswissprot")
KEGG_output_result_pro_2 <- clusterProfiler:: enrichKEGG(gene = as.vector(GO_merge$entrezgene_id), organism = keg_spec_1(), pvalueCutoff = input$cutoff_KEGG, qvalueCutoff = input$qcutoff_KEGG, pAdjustMethod = adjust_method_1(), minGSSize = 1)}  
  })
...
else if(c("Gene Set Enrichment Analysis (GSEA)" %in% input$checkbox_KEGG) && ("Proteomics" %in% input$select_omics_GO_path)){ df_3 <- as.data.frame(Filtertable2_2())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge_1 <- merge(df_3, df_with_Etzgene_1, by.x = "UniProt ID", by.y = "uniprotswissprot")
GO_merge_1 <- as.data.frame(GO_merge_1)
GO_merge_2 <- dplyr::select(.data = GO_merge_1, "entrezgene_id", "logFC") 
...
validate(need(input$submit_KEGG != 0, "1. Treatment-2 will use the same \'Omics Type\', \'pathway analysis type\', \'Pvalue cutoff\', \'q-value cutoff ORA\', and \'pAdjusment method\' provided in the sidebar for treatment 1 (Pathway result-1).

Click on \'Go\' button to perform  pathway enrichment analysis."
))
KEGG_final_table_2()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'), fillContainer = TRUE)
...
else if(c("Gene Set Enrichment Analysis (GSEA)" %in% input$checkbox_KEGG) && ("Proteomics" %in% input$select_omics_GO_path)){ df_3 <- as.data.frame(Filtertable3_3())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge_1 <- merge(df_3, df_with_Etzgene_1, by.x = "UniProt ID", by.y = "uniprotswissprot")
GO_merge_1 <- as.data.frame(GO_merge_1)
GO_merge_2 <- dplyr::select(.data = GO_merge_1, "entrezgene_id", "logFC") 
...
validate(need(input$submit_KEGG != 0, "1. Treatment-3 will use the same \'Omics Type\', \'pathway analysis type\', \'Pvalue cutoff\', \'q-value cutoff ORA\', and \'pAdjusment method\' provided in the sidebar for treatment 1 (Pathway result-1).
Click on \'Go\' button to perform  pathway enrichment analysis."
))
KEGG_final_table_3()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'), fillContainer = TRUE)
...
else if(c("Gene Set Enrichment Analysis (GSEA)" %in% input$checkbox_KEGG) && ("Proteomics" %in% input$select_omics_GO_path)){ df_3 <- as.data.frame(Filtertable4_4())
df_with_Etzgene_1 <- as.data.frame(GO_table_001())
GO_merge_1 <- merge(df_3, df_with_Etzgene_1, by.x = "UniProt ID", by.y = "uniprotswissprot")
GO_merge_1 <- as.data.frame(GO_merge_1)
GO_merge_2 <- dplyr::select(.data = GO_merge_1, "entrezgene_id", "logFC") 
...
validate(need(input$submit_KEGG != 0, "1. Treatment-4 will use the same \'Omics Type\', \'pathway analysis type\', \'Pvalue cutoff\', \'q-value cutoff ORA\', and \'pAdjusment method\' provided in the sidebar for treatment 1 (Pathway result-1).
              
Click on \'Go\' button to perform  pathway enrichment analysis."
))
KEGG_final_table_4()},  options = list(scroller = TRUE, scrollY = 500, scrollX = TRUE, pageLength = 25, selection =  'single', extensions = 'Responsive'), fillContainer = TRUE)
...
fluidRow(column (4, selectInput( inputId = "Colour_high", label = strong("Up"), choices = c("Red","Green", "Yellow", "Blue", "Grey"), selected = "Green", width = '100%')),
column (4, selectInput( inputId = "Colour_med", label = strong("No sign"), choices = c("Red", "Green", "Yellow", "Blue", "Grey"), selected = "Grey", width = '100%')),
column (4, selectInput( inputId = "Colour_low", label = strong("Down"), choices = c("Red", "Green", "Yellow", "Blue", "Grey"), selected = "Red", width = '100%')))

checkboxGroupInput(inputId = "T1", label = h5(strong("Compare treatments")), choices = list("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4"),inline = TRUE)
...
paste0("string_network.png")
}
    },
content <- function(file){
if(all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1) && input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)} 
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1) && input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)} 
else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1) && input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)} 
else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)} 
else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)} 
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)}  
else if(all(c("Treatment-1", "Treatment-3") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)}   
else if(all(c("Treatment-1", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)}   
else if(all(c("Treatment-2", "Treatment-3") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)} 
else if(all(c("Treatment-2", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)}  
else if(all(c("Treatment-3", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA"){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"), file)} 
else if (c(("Treatment-1" %in% input$T1)&& input$checkbox_Pathway == "Pathway ORA")){file.copy(paste0(pathway_ora_plot_1()[1, 1], ".pathview.png"), file)}
  
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1) && input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1) && input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-1", "Treatment-3") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-1", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-2", "Treatment-3") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-2", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if(all(c("Treatment-3", "Treatment-4") %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
else if (c("Treatment-1" %in% input$T1)&& input$checkbox_Pathway == "Pathway GSEA"){file.copy(paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"), file)}
  
else if(input$checkbox_Pathway == "ReactomePA") 
{file.copy(paste0("reactome_", pathway_react_plot_1()[1,1], ".png"), file)} 
else if(input$T1 == "Treatment-1" && input$checkbox_Pathway == "STRING PPI") 
{file.copy(paste0("Img_1.png"), file)}
else if(input$T1 == "Treatment-2" && input$checkbox_Pathway == "STRING PPI") 
{file.copy(paste0("Img_2.png"), file)} 
else if(input$T1 == "Treatment-3" && input$checkbox_Pathway == "STRING PPI") 
{file.copy(paste0("Img_3.png"), file)}
else if(input$T1 == "Treatment-4" && input$checkbox_Pathway == "STRING PPI") 
...
## {data-height="50"}
-----------------------------------------
### To change the pathway re-run the \'Pathway enrichment analysis\'  
  
```{r}

h5(tags$b("To change to another pathway, please re-run the pathway enrichment analysis and then select one enriched pathway in all treatments."), style = "display: inline;color: #000000;")
...
if ("Pathway ORA" %in% input$checkbox_Pathway){
...
if(all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)){

pathway_plot_t1 <- as.data.frame(kegg_pathview_1())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_2()) 
pathway_plot_t3 <- as.data.frame(kegg_pathview_3())  
pathway_plot_t4 <- as.data.frame(kegg_pathview_4())

heatmap_f1 <- merge(pathway_plot_t1, pathway_plot_t2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)

heatmap_f123 <- merge(heatmap_f1, pathway_plot_t3, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)

heatmap_f2 <- merge(heatmap_f123, pathway_plot_t4, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
    
pathway_plot_data_1 <- as.data.frame(replace(heatmap_f2, is.na(heatmap_f2), 0))
...
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1)){
...
heatmap_f2 <- merge(heatmap_f1, pathway_plot_t3, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1)){

pathway_plot_t1 <- as.data.frame(kegg_pathview_1())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_3()) 
pathway_plot_t3 <- as.data.frame(kegg_pathview_4())  
...
else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1)){
...
else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)){

pathway_plot_t1 <- as.data.frame(kegg_pathview_2())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_3()) 
pathway_plot_t3 <- as.data.frame(kegg_pathview_4())  
...
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1)){
pathway_plot_t1 <- as.data.frame(kegg_pathview_1())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_2())
heatmap_f1 <- merge(pathway_plot_t1, pathway_plot_t2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
pathway_plot_data_1 <- as.data.frame(replace(heatmap_f1, is.na(heatmap_f1), 0))
...
else if(all(c("Treatment-1", "Treatment-3") %in% input$T1)){
...
else if(all(c("Treatment-1", "Treatment-4") %in% input$T1)){

pathway_plot_t1 <- as.data.frame(kegg_pathview_1())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_4())
heatmap_f1 <- merge(pathway_plot_t1, pathway_plot_t2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
pathway_plot_data_1 <- as.data.frame(replace(heatmap_f1, is.na(heatmap_f1), 0))
...
else if(all(c("Treatment-2", "Treatment-3") %in% input$T1)){
...
else if(all(c("Treatment-2", "Treatment-4") %in% input$T1)){
...
else if(all(c("Treatment-3", "Treatment-4") %in% input$T1)){

pathway_plot_t1 <- as.data.frame(kegg_pathview_3())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_4())
heatmap_f1 <- merge(pathway_plot_t1, pathway_plot_t2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
pathway_plot_data_1 <- as.data.frame(replace(heatmap_f1, is.na(heatmap_f1), 0))
...
else if("Treatment-1" %in% input$T1){
...
validate(need(input$checkbox_Pathway != "", "1. Please select \'Pathway ORA\' in the sidebar (one at a time only)."),
need(input$T1 != "", "2. Select the color code for log fold change visualization on pathway (\'Up\'- up regulated, \'No sign\'- no significant expression, \'Down\'- down regulated.
3. Select checkbox \'Treatment\' (log fold change value) you want to visualize on the enriched pathway.
4. Click on \'Download\' button to download the image in png format."))  
  

outfile <- reactive({
  
if(all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))} 
else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}
else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-3") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-2", "Treatment-3") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}
else if(all(c("Treatment-2", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.multi.png"))} 
else if ("Treatment-1" %in% input$T1){normalizePath(path = paste0(pathway_ora_plot_1()[1, 1], ".pathview.png"))}
})
pathway_plot_1()
...
if ("Pathway GSEA" %in% input$checkbox_Pathway){  
...
pathway_plot_t1 <- as.data.frame(kegg_pathview_gsea_1())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_gsea_2()) 
pathway_plot_t3 <- as.data.frame(kegg_pathview_gsea_3())  
pathway_plot_t4 <- as.data.frame(kegg_pathview_gsea_4())
...
p3.out <- pathview(gene.data = ORA_path_mtrx, pathway.id = pathway_gsea_plot_1()[1, 1], species = keg_spec_1(), limit = list(gene = max(abs(round(ORA_path_mtrx, 0)))), bins = list(gene= 10), low = input$Colour_low , mid = input$Colour_med, high = input$Colour_high)
}  
...
pathway_plot_t1 <- as.data.frame(kegg_pathview_gsea_1())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_gsea_3()) 
pathway_plot_t3 <- as.data.frame(kegg_pathview_gsea_4())  
...
pathway_plot_t1 <- as.data.frame(kegg_pathview_gsea_2())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_gsea_3()) 
pathway_plot_t3 <- as.data.frame(kegg_pathview_gsea_4())  
...
p2.out <- pathview(gene.data = ORA_path_mtrx, pathway.id = pathway_gsea_plot_1()[1, 1], species = keg_spec_1(), limit = list(gene = max(abs(round(ORA_path_mtrx, 0)))), bins = list(gene= 10), low = input$Colour_low , mid = input$Colour_med, high = input$Colour_high) 
}  
...
pathway_plot_t1 <- as.data.frame(kegg_pathview_gsea_1())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_gsea_4())
heatmap_f1 <- merge(pathway_plot_t1, pathway_plot_t2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
pathway_plot_t1 <- as.data.frame(kegg_pathview_gsea_3())  
pathway_plot_t2 <- as.data.frame(kegg_pathview_gsea_4())
heatmap_f1 <- merge(pathway_plot_t1, pathway_plot_t2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
pathway_plot_data_1 <- as.data.frame(replace(heatmap_f1, is.na(heatmap_f1), 0))
...
pathway_plot_data_1 <- as.data.frame(kegg_pathview_gsea_1())
ORA_path_mtrx <- as.matrix(pathway_plot_data_1[, -1])
row.names(ORA_path_mtrx) <- pathway_plot_data_1[, 1]   
    
p1.out <- pathview(gene.data = ORA_path_mtrx, pathway.id = pathway_gsea_plot_1()[1, 1], species = keg_spec_1(), limit = list(gene = max(abs(round(ORA_path_mtrx, 0)))), bins = list(gene= 10), low = input$Colour_low , mid = input$Colour_med, high = input$Colour_high) 
}
})
output$pathway_gsea_plot <- renderImage({
  
validate(need(input$checkbox_Pathway != "", "1. Please select \'Pathway GSEA\' in Pathway visualization (one at a time only).
2. Select the color code for log fold change visualization on pathway (\'Up\'- up regulated, \'No sign\'- no significant expression, \'Down\'- down regulated."),
need(input$T1 != "", "3. Select the \'Treatment\' (log fold change value) you want to visualize on the enriched pathway.
4. Click on \'Download\' button to download the image in png format."))  
  
outfile <- reactive({
  
if(all(c("Treatment-1", "Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}
else if(all(c("Treatment-1", "Treatment-2", "Treatment-3") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))} 
else if(all(c("Treatment-1", "Treatment-2", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}
else if(all(c("Treatment-2", "Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-2") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-3") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-1", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-2", "Treatment-3") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}
else if(all(c("Treatment-2", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))}  
else if(all(c("Treatment-3", "Treatment-4") %in% input$T1)){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.multi.png"))} 
else if ("Treatment-1" %in% input$T1){normalizePath(path = paste0(pathway_gsea_plot_1()[1, 1], ".pathview.png"))}
})
...
if ("ReactomePA" %in% input$checkbox_Pathway){
...
react_1 <-  as.data.frame(reactome_pathview_1())
react_2 <-  as.data.frame(reactome_pathview_2())
react_3 <-  as.data.frame(reactome_pathview_3())
react_4 <-  as.data.frame(reactome_pathview_4())

heatmap_f <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)  
heatmap_f123 <- merge(heatmap_f, react_3, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)

heatmap_f1 <- merge(heatmap_f123, react_4, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)

react_data_1 <-  as.data.frame(heatmap_f1)
...
react_3 <-  as.data.frame(reactome_pathview_3())
heatmap_f <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)  
heatmap_f1 <- merge(react_3, heatmap_f, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
react_1 <-  as.data.frame(reactome_pathview_1())
react_2 <-  as.data.frame(reactome_pathview_2())
react_3 <-  as.data.frame(reactome_pathview_4())
heatmap_f <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)  
heatmap_f1 <- merge(react_3, heatmap_f, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
react_1 <-  as.data.frame(reactome_pathview_1())
react_2 <-  as.data.frame(reactome_pathview_3())
react_3 <-  as.data.frame(reactome_pathview_4())
heatmap_f <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)  
heatmap_f1 <- merge(react_3, heatmap_f, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
react_1 <-  as.data.frame(reactome_pathview_2())
react_2 <-  as.data.frame(reactome_pathview_3())
react_3 <-  as.data.frame(reactome_pathview_4())
heatmap_f <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)  
heatmap_f1 <- merge(react_3, heatmap_f, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
react_1 <-  as.data.frame(reactome_pathview_1())
react_2 <-  as.data.frame(reactome_pathview_2())
heatmap_f1 <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
react_1 <-  as.data.frame(reactome_pathview_1())
react_2 <-  as.data.frame(reactome_pathview_4())
heatmap_f1 <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
react_1 <-  as.data.frame(reactome_pathview_3())
react_2 <-  as.data.frame(reactome_pathview_4())
heatmap_f1 <- merge(react_1, react_2, by.y = "geneID", by.x = "geneID", all = TRUE) %>% replace(is.na(.), 0)
...
validate(need(input$checkbox_Pathway != "", "1. Please select \'ReactomePA\' in Pathway visualization (one at a time only).
2. Select the color code for log fold change visualization on pathway (\'Up\'- up regulated, \'No sign\'- no significant expression, \'Down\'- down regulated."),
need(input$T1 != "", "3. Select the \'Treatment\' to map log fold change value on the selected enriched pathway.
4. Click on \'Download\' button to download the image in png format."))
...
validate(need(input$checkbox_Pathway != "", "1. Please select \'STRING PPI\' in Pathway visualization (one at a time only)."),
need(input$T1 != "", "2. Select the \'Treatment\' (one at a time only) to map the log fold change value on the protein protein interaction network.
4. Click on \'Download\' button to download the image in png format."))  
...
rbPal <- colorRampPalette(c("blue", "pink", "red"))
...
legend("topright", legend = c("up", "no sign", "down"), col = c("red", "pink", "blue"), title = "regulation (Halo)", lty = 10,lwd = 18, bty = "n")
...
div(h6(tags$b("ALERT:- Please select only one row from the result table. Selected row will be visualize in the Abstract info window"), style = "display: inline;color: #000000;"))
...
```{r, echo= FALSE}
literature_search <- eventReactive(input$literature_submit, {
pmc_table <- europepmc::epmc_search(query = input$text_literature, limit = input$literature_limit, sort = "date")
pmc_table_1 <- pmc_table[, c(1,3,5,6,7,10)]
  })

output$literature_result_1 <- DT::renderDataTable({
validate(need(input$text_literature != "", "1. Please provide input for \'Literature search\', example- Gene name (IL6), species (Human), disease type (Cancer), etc."),   
need(input$literature_limit != "", "2. Provide input for \'limiting the retrieval\' of scientific lietrature; example 100-5000."),
need(input$literature_submit, "3. Click on \'Submit\' button to perform retrieval!")) 
...
literature_data_1 <- literature_data[1, 2]
...
validate(need(input$literature_result_1_rows_selected != "", "Select only one row at a time from \'Literature info\' result table."))  
...
h3("INSTRUCTIONS")
p("•	The app is hosted on shiny.io website:", strong("https://omnalysis.shinyapps.io/OMnalysis/"), a( "click here", href="https://omnalysis.shinyapps.io/OMnalysis/", target="_blank"))
p("•	Codes are freely available at Github:", strong("https://github.com/Punit201016/OMnalysis"), a("click here", href="https://github.com/Punit201016/OMnalysis/", target="_blank"))
p("•	Step-by-step guide can be acessed by:", strong("https://github.com/Punit201016/OMnalysis"), a("click here", href="https://github.com/Punit201016/OMnalysis", target="_blank"))
