...
div(.progress = {"height: 0px;"}, fileInput(inputId = "inFile1", label = h5(strong("Upload Expression Data")),
multiple = TRUE, accept = c(
                    "text/csv",
                    "text/comma-separated-values",
                    "text/plain",
...
content = function(file){
if(input$PCA_M == TRUE){ggsave(file, plot(Plot_PCA_1()), device = input$select_PCA, width = input$Download_PCA_input1, height = input$Download_PCA_input2, units = "cm", dpi = input$Download_PCA_input3)}
...
if(input$checkbox_Pathway == "STRING PPI") 
{file.copy("string_network_1.png", file)}
  },
...
string_table_1()$plot_network(string_net_2, payload_id = payload_id)
legend("topright", legend = c("up", "no sign", "down"), col = c(input$Colour_high, input$Colour_med, input$Colour_low), lty = 10,lwd = 18,bty = "n")
legend("bottomleft", legend = c("from curated databases", "experimentally determined", "gene neighborhood", "gene fusions", "gene co-occurrence", "textmining", "co-expression", "protein homology"), col = c("deepskyblue1", "pink", "green", "red", "blue3", "darkolivegreen1", "black", "darkorchid1"), title = "Interactions", lty = 5,lwd = 3, bty = "n")
